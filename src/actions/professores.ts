...
export async function buscarProfessoresComTurmas() {
  const [professores, turmas] = await Promise.all([
    sql`
      SELECT p.id, p.nome, p.especialidade, p.telefone, p.email, p.data_ingresso, p.turma_aluno_id, p.cargo,
  ...
  return {
    professores: professores.map(p => ({
      ...
      email: p.email, data_ingresso: p.data_ingresso, turma_aluno_id: p.turma_aluno_id,
      cargo: p.cargo, turma_ids: p.turma_ids ?? [],
    })),
...
export async function salvarProfessor(dados: {
  ...
  try {
    ...
    if (dados.id) {
      ...
      const [row] = await sql`
        INSERT INTO professores (nome, especialidade, telefone, email, turma_aluno_id, cargo, data_ingresso, ativo)
        VALUES (${dados.nome}, ${dados.especialidade ?? null}, ${dados.telefone ?? null}, ${dados.email ?? null}, ${dados.turma_aluno_id ?? null}, ${dados.cargo ?? null}, ${new Date().toISOString().split('T')[0]}, true)
        RETURNING id
      `
      profId = row.id
    }
...
export async function sincronizarAlunoVinculado(profId: string, nome: string, turmaAluno: string | null) {
  const marcador = `professor:${profId}`
  const existente = await sql`SELECT id FROM alunos WHERE responsavel = ${marcador} LIMIT 1`

  if (turmaAluno) {
    if (existente.length > 0) {
      await sql`UPDATE alunos SET nome = ${nome}, turma_id = ${turmaAluno}, ativo = true WHERE id = ${existente[0].id}`
    } else {
      await sql`INSERT INTO alunos (nome, turma_id, responsavel, ativo) VALUES (${nome}, ${turmaAluno}, ${marcador}, true)`
    }
